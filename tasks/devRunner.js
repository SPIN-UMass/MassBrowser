...
rendererHotDevProcess = run(
  rendererHotDevPanel,
  `webpack-dev-server --hot --colors --config tasks/webpack/webpack.${TARGET}.electron.renderer.js --mode development  --port ${PORT} --content-base app/dist/${TARGET}`,
  data => data.indexOf('Compiled successfully') >= 0
)
mainWatchProcess = run(
  mainWatchPanel,
  `webpack --mode development --watch --colors --config tasks/webpack/webpack.${TARGET}.electron.main.js`
)
